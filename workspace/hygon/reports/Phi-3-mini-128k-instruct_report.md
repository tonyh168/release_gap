...
- **部署侧根因同批一致**：容器内默认 clang 路径不匹配会触发 Triton/FlagGems 编译问题，显式设置 `TRITON_HIP_CLANG_PATH=/opt/dtk-26.04-DCC2602-0317/aillvm/bin/clang-18` 后服务稳定；只用 `HIP_VISIBLE_DEVICES` 控卡，不设置 `ROCR_VISIBLE_DEVICES`（后者会让 `torch.cuda.is_available()` 变 `False`）。
...
/public-flash/models/release_run_logs/Phi-3-mini-128k-instruct/enabled_ops_retry-clang18-20260917.txt
```
...
export VLLM_FL_TRITON_CACHE_ROOT=/models/release_run_logs/triton_cache/Phi-3-mini-128k-instruct
export FLAGGEMS_ENABLE_OPLIST_PATH=/models/release_run_logs/Phi-3-mini-128k-instruct/enabled_ops_retry-clang18-20260917.txt
vllm serve /models/flagrelease/fixes_models/Phi-3-mini-128k-instruct \

...
原始服务运行在共享容器 `flagrelease_thead_model_dl_20260915`。以下使用独立复现容器名；执行前确认 GPU6 和端口 `18080` 空闲。

```bash
set -euo pipefail

IMAGE="harbor.baai.ac.cn/flagrelease-public/qwen3.8-27b-pp001-gems0.0-treenone-cxnone-plugin0.2.0-vllm0.24.0-cp312-pt210-hggc130-x64-1.3.2-d7f5a2:202608141100"
CONTAINER="flagrelease_thead_qwen3_4b_thinking_2507_repro"
...
```bash
docker exec -d flagrelease_thead_qwen3_4b_thinking_2507_repro bash -lc '
set -euo pipefail

MODEL_ROOT="${MODEL_ROOT:-/models}"
MODEL_PATH="${MODEL_PATH:-${MODEL_ROOT}/Qwen3-4B-Thinking-2507}"
CACHE_ROOT="${CACHE_ROOT:-${MODEL_ROOT}/_vllm_cache/qwen3-4b-gpu6}"
LOG_ROOT="${LOG_ROOT:-${MODEL_ROOT}/_serve_logs}"
PPU_SDK_ROOT="${PPU_SDK_ROOT:-/usr/local/PPU_SDK}"
TARGET_DEVICES="${TARGET_DEVICES:-6}"
...
exec /usr/local/bin/vllm serve "${MODEL_PATH}" \
  ...
  --trust-remote-code \
  --enforce-eager \
  > "${LOG_ROOT}/Qwen3-4B-Thinking-2507-repro-gpu6-port18080.log" 2>&1
'
```

`qwen3-4b-gpu6` 是默认缓存命名空间，不是固定机器目录；命令自动创建，也可通过 `CACHE_ROOT`、`TARGET_DEVICES` 覆盖。
...
docker exec flagrelease_thead_qwen3_4b_thinking_2507_repro \
  bash -lc "pgrep -af '^/usr/local/bin/python3.12 /usr/local/bin/vllm serve /models/Qwen3-4B-Thinking-2507'; ppu-smi"
```

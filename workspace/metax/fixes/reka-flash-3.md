...
|------|---|
| MetaX 最终得分（v6） | **54.04%** |
| 达标基准（**NV 原生实测**：H20 + vLLM 官方镜像 + FA3） | **53.54%** |
| 差值 | **+0.50pt（反超）** |
...
- **注意与手动复现文档的口径差异**：`Reka-Flash-3_GPQA-Diamond_手动复现.md` 中的
  NV origin 一轮，请求里显式传的是 `temperature=0.0 / top_p=1.0 / top_k=-1`，
  **不是**模型自带的 0.6。本轮改用 `generation_config.json` 后，
  采样温度是与该文档**唯一的配置差异**，因此对比基准时应记得这一项（见后文）。
...
2. **显著优于同为沐曦环境的早期贪心轮次**（v3=46%、v4 前 115 题=46.96%）。
   这两轮的黑名单与 v6 **完全相同**，唯一变量是采样参数，
   说明沐曦侧没有出现额外的精度损失，退化全部来自评测参数。
...
3. **显著高于本模型此前的贪心轮次**（v3=46%、v4 前 115 题=46.96%）。
   即：本次配置既没有出现异常退化，也确实优于此前的贪心配置。
...
1. **53.54% 是亲手复现值**。`Reka-Flash-3_GPQA-Diamond_手动复现.md` 记录的 NV origin
   ...
   **106/198 = 53.54%**，与 MetaX v6 的 107/198 = 54.04% 同题同量，
   是唯一可直接对比的基准。已确认无需再用 NV vllm 官方镜像复核。
   （两者唯一的配置差异是采样温度：NV 那轮为贪心 0.0，本轮为模型自带的 0.6——
   该差异**对本轮有利**，因此「反超 0.50pt」应保守地理解为「与 NV 原生等同」，
   见「精度缺口归因」一节。）

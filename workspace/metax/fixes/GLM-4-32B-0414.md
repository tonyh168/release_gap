...
|------|--------------------------|------------|------------------------|------|
| v1（裸 vLLM） | N/A | — | — | serve_v1.log，未跑评测 |
| v2（plugin-FL 默认黑名单） | mm,mm_out,bmm,bmm_out,linear,sort,stable_sort,masked_fill,masked_fill_ ,slice | **52%** (26/50) | **0（达标）** | 2026-09-15T02:46:07 |
...
原始 predictions 文件：`outputs/gpqa_diamond/20260915_023318/predictions/GLM-4-32B-0414/gpqa_diamond_default.jsonl`（eval-scope 容器内）

---

## 现象

原报告 V1–V4 全空（服务启动阶段就崩）。本次用 plugin-FL 默认黑名单直接起 v2 服务，日志无崩溃，`Application startup complete` 正常，长 prompt 冒烟通过。v2 GPQA 52%（26/50 题正确）。

## 定位

plugin-FL 默认黑名单（`mm,mm_out,bmm,bmm_out,linear,sort,stable_sort,masked_fill,masked_fill_,slice`）覆盖了 GLM-4 的崩溃算子，eager 模式下服务一次性起成功，无需额外调整黑名单。精度对比 NV 基线 55%，绝对差 1.5 题，属小样本方差，非算子精度退化。

## 处置

直接用 SOP 默认黑名单 + `VLLM_FL_USE_FLAGGEMS_ATTN=0` + `--enforce-eager`，TP=2（GPU 0,1），端口 8003，`max_model_len=32768`。无需二分排查。

## 结果

- 修复后 GPQA 正确率：**52%** (26/50)
- NV 基线：55%
- 达标判定（accuracy_compare 退出码）：**0（达标，noise_zone=true，1.5 题差 ≤ 2 题阈值）**

## 提炼到 KNOWLEDGE 的条目

GLM-4-32B-0414 在 MetaX 上用 plugin-FL 默认黑名单 + eager 模式可一次起成功，无需额外补充黑名单条目。

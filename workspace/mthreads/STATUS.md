...
> 更新：2026-09-21
> ✅ **50 题筛查 4 个模型全部出分** —— 3 个稳妥达标；**reka-flash-3 出现「两簇」未归因**，
> 判定依基准而定（53.54 → 达标 / 59 → 不达标）。明细见下「📊 50 题筛查结果」
> 🔬 **reka-flash-3 重复性实验已完成**：5 轮同配置评测落成 `50,50 | 58,58,58` 两簇，均值 54.8%，
> **分簇原因未查明**（老容器已停，未继续追查）。见下「🔬 reka-flash-3 重复性实验」
...
| **50 题筛查跑分** | ✅ **已完成** —— 3 个稳妥达标；reka 出现两簇未归因，判定依基准而定，见下「📊 50 题筛查结果」 |
...
| **reka-flash-3** | **54.8%**（5 轮均值，区间 50.0–58.0%） | 59 / **53.54**※ | **+7.1%** / **−2.4%** | ⚠️ **基准决定结论** | 0 | 无 | [[reports/reka-flash-3_report]] |

> ※ **reka-flash-3 的基准按 metax 裁定改用 NV 原生实测 53.54**（表内 59 出自 NV 失败报告、口径不符）。
> ⚠️ **两个基准相差 5.5pt，而本模型 50 题口径的噪声就有 8pt** —— **基准选择直接翻转结论**：
> 按 **53.54** → 反超 2.4% **✅ 达标**；按表内 **59** → 退化 7.1% **❌ 不达标**。
> 本表状态记为「依 53.54 判达标」，完整口径限定写在 [[reports/reka-flash-3_report]] 的发布字段注记里。
>
...
- **reka-flash-3** —— ⚠️ **5 轮同配置评测落成「两簇」（50,50 | 58,58,58），分簇原因未查明**，
  均值 54.8%。判定**依基准而定**：53.54 → 达标；59 → 不达标。老容器已停，**定稿须走 198 题全量**。详见下节。
...
| 基准 | 54.8% 的判定 |
|------|--------------|
| metax 裁定的 NV 原生 **53.54** | 反超 2.4% → ✅ **达标** |
| 表内 **59** | 退化 **7.1% > 5%** → ❌ **不达标** |

> **两个基准相差 5.5pt，而 50 题的噪声就有 8pt** —— 基准选择直接翻转结论。
> 本表按 **53.54** 判达标，口径限定见 [[reports/reka-flash-3_report]] 发布字段注记。
...
| reka-flash-3 | gpqa_diamond 59（**裁定改用 53.54**） | ⚠️ 依基准而定 | ⚠️ **5 轮同配置评测落成两簇**（`50,50 \| 58,58,58`），均值 **54.8%**，**分簇原因未查明**（老容器已停）。判定：按 **53.54** → 反超 2.4% ✅；按表内 **59** → 退化 7.1% ❌。采样参数一直生效、算子侧无需改动（metax 那条根因在摩尔侧**从未发生**）。⚠ 50 题口径噪声达 8pt，**定稿必须走 198 题全量**。报告：[[reports/reka-flash-3_report]] |
...
- **精度已通过**：**3 / 50**（50 题筛查口径，结论稳妥）—— `Phi-4-reasoning-plus` /
  `LFM2.5-1.2B-Thinking` / `Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled`
- **精度待定**：**1 / 50** —— `reka-flash-3`（两簇未归因；依 53.54 判达标、依 59 判不达标）
- **精度不达标**：**0 / 50**（在 53.54 基准下）
...
| 跑分进度 | ✅ **50 题筛查已完成**（4/4 出分：3 个稳妥达标 + reka 待定）；⬜ **198 题全量待跑** |
...
| 3 | reka-flash-3 判定基准改用 **NV 原生实测 53.54**（非表中 59） | 仅该模型 | metax 基准取值裁定 | ✅ 已执行 —— ⚠️ **基准决定结论**：54.8% 对 53.54 反超 2.4%（达标）、对 59 退化 7.1%（不达标） |

...
|------|----|
| 得分 | **50.0%（25/50）** |
| NV 基线 | 表内 **59** → 退化 **+15.25%**；按 metax 裁定采用 NV 原生 **53.54** → 退化 **−6.61%** |
| 判定（当时） | ❌ 退出码 1（**注**：基准已改按 53.54，但这一轮 50.0% 在该基准下仍不达标 → 该结果后被 5 轮重复实验取代，见文首「最终结论」） |
...
| **判定基准** | **采用 NV 原生 198 题实测 53.54%**（metax 裁定，本报告沿用同一基准以便跨平台对比），<br>**不用**表里的 59 | 53.54% 是 NV 原生（H20 + vLLM 官方镜像 + FA3、无 plugin、198 题）亲手复现值；<br>59 出自 NV 失败报告，而同报告记录 NV 硬件上 plugin-FL 同样退化 12pt（60→48）→ 口径不符 |

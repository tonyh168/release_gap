...
- 修复后分 / NV 基线：**54.8%（5 轮均值，单轮区间 50.0% ~ 58.0%）/ 53.54%**
  （NV 原生 198 题实测，基准裁定见下）→ **反超 1.26pt（相对 +2.35%）**
...
### 基准取值（采用 metax 的裁定：53.54%，不用表内 59）

**裁定依据沿自 metax（2026-09-19 裁定，本报告沿用同一基准以便跨平台对比）**：

1. **53.54% 是亲手复现值**：NV origin 一组（H20 + vLLM 官方镜像 + FA3，**无 plugin**，
   198 题全量）实测 **106/198 = 53.54%**，是唯一与摩尔口径可直接对比的基准。
2. **59.0 的出处与口径均不支持直接比较**：该值出自
   `flagrelease_fail_reports/Nvidia/FAILED_Nvidia_reka-flash-3_202607300901.md`，而**同一份报告**
   记录 NV 硬件上 V2（gems 无 plugin，50 题）= **60.0%**、V3（plugin，50 题）= **48.0%** ——
   即 ①59.0 与「198 题、无 plugin」的原生复现不是同一口径；
   ②**NV 硬件上 plugin-FL 同样造成 12pt 退化**，该模型对 plugin-FL 敏感是**跨平台共性**。
3. 与 `SOLAR-10.7B-Instruct-v1.0` 属同类情况，处置一致：**基线表取值与实测不符时以实测为准**。

> ⚠️ **对上一版报告的修正**：本报告曾采用表内 **59** 作 `SCORE_ORIGIN` 并据此判定「不达标」，
> 现统一改用 metax 裁定的 **53.54**。两者相差 5.5pt，而本模型 50 题口径的噪声就有 8pt ——
> **基准取值直接决定结论**，故此处明确记录取值理由，不再并列两种口径造成歧义。

### 为什么必须用 198 题全量定稿

metax 侧已算出 **50 题口径的标准误约 ±7.1pt（95% 置信区间约 ±14pt）**，
本平台的实测摆动（8pt，见「定位」第 3 条）**与之完全吻合** —— 两家独立得到同一量级的噪声估计。
即：**50 题口径的噪声带宽比 5% 判定容差还宽**，任何单轮 50 题结论都不可采信。
...
>
> 注记二（`SCORE_ORIGIN: 53.54` 的取值）：采用 metax 裁定的 **NV 原生 198 题实测 53.54%**
> （详见正文「基准取值」小节），**不采用** `nv_baseline.yaml` 表内的 `gpqa_diamond: 59`
> —— 该值出自 NV 失败报告，与「198 题、无 plugin」的原生复现不是同一口径。
> 此基准下 54.8% 反超 1.26pt（相对 +2.35%），判定达标。
